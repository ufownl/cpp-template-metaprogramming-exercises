...
struct binary_tree_search
	: eval_if<
		equal_to<T, typename root<_tree>::type>,
		bool_<true>,
		eval_if<
			less<T, typename root<_tree>::type>,
			binary_tree_search<typename left_child<_tree>::type, T>,
			binary_tree_search<typename right_child<_tree>::type, T>
		>
	>
{
};
...
template <class T>
struct binary_tree_search<none, T> : bool_<false>
{
};
...
int main(int argc, char *argv[])
{
	...
	BOOST_STATIC_ASSERT((binary_tree_search<bst, int_<11> >::type::value));
	BOOST_STATIC_ASSERT((!binary_tree_search<bst, int_<20> >::type::value));
	return 0;
}